...
# convert singles (TxHxWxC) to 4-dimensional torch tensor
def singles2tensor4(imgs):
    return torch.from_numpy(np.ascontiguousarray(imgs)).permute(0, 3, 1, 2).float()
...
def random_crop(lqs, hq, sf=4, lq_patchsize=64):
    h, w = lq.shape[:2]
